...
public abstract class AmqpClient {
    ...
    private static String accessKey = "LTAI5tMZvanqKAJvubTGdPAe";
    private static String accessSecret = "WXHLo4zOEfpIzvoIgbCTZs5vI25sxV";
    private static String consumerGroupId = "VGDnXnVuKoQEu7nzncCU000100";

    //iotInstanceId：实例ID。若是2021年07月30日之前（不含当日）开通的公共实例，请填空字符串。
    private static String iotInstanceId = "iot-06z00id0125zq0m";

    //控制台服务端订阅中消费组状态页客户端ID一栏将显示clientId参数。
    //建议使用机器UUID、MAC地址、IP等唯一标识等作为clientId。便于您区分识别不同的客户端。
    private static String clientId = "172.19.134.9";

    //${YourHost}为接入域名，请参见AMQP客户端接入说明文档。
    private static String host = "1864755956607862.iot-amqp.cn-shanghai.aliyuncs.com";
    ...
    private final static Thread executorService = new Thread();
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!executorService.isAlive()) {
            logger.info("shutdown success");
            for(Iterator a=strArray.iterator();a.hasNext();){
     ...
     * 在这里处理您收到消息后的具体业务逻辑。
     */
    static ArrayList strArray= new ArrayList();
    ...
    public static void processMessage(Message message) {
        try {
            ...
            //  if(Integer.parseInt(content)>1600&&Integer.parseInt(content)<3000){
